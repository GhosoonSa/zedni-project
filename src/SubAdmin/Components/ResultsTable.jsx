...
const ResultsTable = ({ students, subjectId }) => {
    ...
    const fetchMarks = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8000/api/subadmin/getMarksSubadmin/${subjectId}`,
          {
            headers: {
  ...
  const calculateStats = () => {
    const statusObj = marks?.status || {};
    const passed = Object.values(statusObj).filter(
    ...
    return {
      ...
      passedPercentage:
        totalStudents > 0 ? ((passed / totalStudents) * 100).toFixed(1) : "0.0",
      failedPercentage:
        totalStudents > 0
          ? (((totalStudents - passed) / totalStudents) * 100).toFixed(1)
          : "0.0",
      totalStudents,
    };
  };
          ...
          <TableBody>
            {students.map((student) => (
              <TableRow key={student.studentID} hover>
                ...
                <TableCell>{marks.study[student.studentID] ?? "-"}</TableCell>
                <TableCell>{marks.exam[student.studentID] ?? "-"}</TableCell>
                <TableCell>
