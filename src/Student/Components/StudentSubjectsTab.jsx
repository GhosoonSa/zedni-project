    ...
    <Box sx={{ p: 3 }}>
      ...
      {selectedSubject && (
        <Box sx={{
          ...
          {}
          {selectedSubject.showAttachments && selectedSubject.attachments.length > 0 && (
            <Box sx={{ mb: 4 }}>
              <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: '#5a3e1b', mb: 2 }}>
                الملحقات:
              </Typography>
            ...
            {selectedSubject.recommendedBooks.length > 0 ? (
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {selectedSubject.recommendedBooks.map((book) => (
                  ...
                  >
                    <MenuBookIcon color="primary" sx={{ fontSize: '2rem', mr: 1 }} />
                    <Typography 
                      variant="body1" 
                      sx={{ 
                        fontWeight: 'bold',
                    ...
                    <Chip
                      label={book.isRecommended ? " تمت التوصية" : " توصية كتاب"}
                      onClick={() => handleBookRecommendation(selectedSubject.id, book.id)}
                      color={book.isRecommended ? "success" : "default"}
                      sx={{
                        cursor: 'pointer',
                        backgroundColor: book.isRecommended ? '#4caf50' : '#7a6248',
                        color: book.isRecommended ? 'white' : 'white',
                        '&:hover': {
                          backgroundColor: book.isRecommended ? '#388e3c' : '#d5c9b1'
                        },
                        ml: 1,
                        minWidth: '80px',
                          fontWeight: 'bold'
              ...
              <Typography variant="body1">لا توجد كتب متاحة للتوصية حالياً</Typography>
            )}
          </Box>
        </Box>
      )}
